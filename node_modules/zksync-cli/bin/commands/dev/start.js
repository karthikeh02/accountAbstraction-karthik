        ...
        const interval = setInterval(async () => {
            try {
                if (tries >= maxTries) {
                    ...
                    spinner.fail(`Can't connect to "${nodeInfo.name}" ${chalk.gray(`(${nodeInfo.rpcUrl})`)}. Please make sure it is running.`);
...
const startModules = async (modules) => {
...
const stopOtherNodes = async (currentModules) => {
    ...
    for (const module of modules) {
        if (module.category === ModuleCategory.Node &&
...
const checkForUpdates = async (modules) => {
    ...
    if (modulesRequiringUpdates.length > 1) {
        Logger.info(chalk.gray(`Update all modules: npx zksync-cli dev update ${modulesRequiringUpdates
            .map(({ module }) => module.package.name)
            .join(" ")}`));
    }
};
const showStartupInfo = async (modules) => {
    for (const module of modules) {
        Logger.info("");
