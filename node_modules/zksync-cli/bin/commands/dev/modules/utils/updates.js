...
const getModuleUpdatesInfo = async () => {
...
const checkModulesForUpdates = async (modules) => {
    const moduleUpdatesInfo = await getModuleUpdatesInfo();
    let hadChanges = false;
    await Promise.all(modules.map(async (module) => {
        const moduleInfo = moduleUpdatesInfo[module.package.name];
        if (moduleInfo) {
            const hoursSinceLastUpdate = (new Date().getTime() - new Date(moduleInfo.lastUpdateCheck).getTime()) / (1000 * 3600);
...
export const getModulesRequiringUpdates = async (modules) => {
