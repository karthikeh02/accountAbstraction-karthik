...
import { askForTemplate, setupTemplate, getUniqueValues, askForPackageManager, successfulMessage } from "../utils.js";
...
export default async (folderLocation, folderRelativePath, templateKey) => {
    ...
    if (!templateKey) {
        ...
        const { ethereumFramework } = await inquirer.prompt([
            {
                message: "Ethereum framework",
                name: "ethereumFramework",
                type: "list",
                choices: getUniqueValues(templates.filter((template) => template.framework === framework).map((template) => template.ethereumFramework)),
                required: true,
            },
        ]);
        template = await askForTemplate(templates
    ...
    if (template.requiresWalletConnectProjectId) {
        const { walletConnectProjectId } = await inquirer.prompt([
            {
