...
import { decodeData, encodeData, encodeParam, getFragmentFromSignature, getInputValues, getInputsFromSignature, } from "./utils/formatters.js";
...
const askOutputTypes = async (rawCallResponse, options) => {
    ...
    const answers = await inquirer.prompt([
        {
            ...
            validate: (input) => {
                try {
                    ...
                    return `${chalk.redBright("Failed to decode response with provided types: " + (error instanceof Error ? error.message : error))}\nInput example: ${chalk.blueBright("string,uint256")}`;
                }
            },
        },
    ], options);
    options.outputTypes = options.outputTypes || getInputValues(answers.outputTypes);
    if (!options.outputTypes.length)
        return;
