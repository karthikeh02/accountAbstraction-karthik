...
export default class SetupModule extends Module {
    ...
    getInstalledModuleFilePath(fileName) {
    ...
    async applyAppConfig() {
        ...
        const commandError = await helpers.executeCommand(`docker compose cp ${appConfigPath} app:${APP_RUNTIME_CONFIG_PATH}`, { silent: true, cwd: this.dataDirPath });
        if (commandError) {
    ...
    async install() {
        try {
            ...
            if (error?.toString().includes("operation not permitted")) {
                throw new Error("Not enough permissions to create necessary files. Please run console in administrator mode and try again.");
            }
            throw error;
        }
    }
    ...
    async cleanupIndexedData() {
        await Promise.all(["worker", "api", "postgres"].map((serviceName) => helpers.executeCommand(`docker compose rm -fsv ${serviceName}`, { silent: true, cwd: this.dataDirPath })));
    ...
    async getNodeLatestBlockNumber() {
        const nodeInfo = await this.configHandler.getNodeInfo();
        try {
